...
class CParser(PLYParser):
    ...
    def p_type_specifier_1(self, p):
        """ type_specifier  : VOID
                            ...
                            | UNSIGNED
        """
        p[0] = c_ast.IdentifierType([p[1]], coord=self._yacccoord(p,1))
    ...
    def p_enum_specifier_1(self, p):
        """ enum_specifier  : ENUM ID
                            | ENUM TYPEID
        """
        p[0] = c_ast.Enum(p[2], None, self._yacccoord(p,1))

    def p_enum_specifier_2(self, p):
        """ enum_specifier  : ENUM brace_open enumerator_list brace_close
        """
        p[0] = c_ast.Enum(None, p[3], self._yacccoord(p,1))

    def p_enum_specifier_3(self, p):
        """ enum_specifier  : ENUM ID brace_open enumerator_list brace_close
                            | ENUM TYPEID brace_open enumerator_list brace_close
        """
        p[0] = c_ast.Enum(p[2], p[4], self._yacccoord(p,1))
    ...
    def p_direct_declarator_5(self, p):
        ...
        arr = c_ast.ArrayDecl(
            type=None,
            dim=c_ast.ID(p[4], self._yacccoord(p,4)),
    ...
    def p_parameter_type_list(self, p):
        ...
        """
        if len(p) > 2:
            p[1].params.append(c_ast.EllipsisParam(self._yacccoord(p,3)))
    ...
    def p_initializer_2(self, p):
        ...
        """
        if p[2] is None:
            p[0] = c_ast.InitList([], self._yacccoord(p,1))
        else:
            p[0] = p[2]
    ...
    def p_direct_abstract_declarator_4(self, p):
        ...
        arr = c_ast.ArrayDecl(
            type=None,
            dim=c_ast.ID(p[3], self._yacccoord(p,3)),
            dim_quals=[],
            coord=p[1].coord)
    ...
    def p_labeled_statement_1(self, p):
        ...
        p[0] = c_ast.Label(p[1], p[3], self._yacccoord(p,1))

    def p_labeled_statement_2(self, p):
        """ labeled_statement : CASE constant_expression COLON statement """
        p[0] = c_ast.Case(p[2], [p[4]], self._yacccoord(p,1))

    def p_labeled_statement_3(self, p):
        """ labeled_statement : DEFAULT COLON statement """
        p[0] = c_ast.Default([p[3]], self._yacccoord(p,1))

    def p_selection_statement_1(self, p):
        """ selection_statement : IF LPAREN expression RPAREN statement """
        p[0] = c_ast.If(p[3], p[5], None, self._yacccoord(p,1))

    def p_selection_statement_2(self, p):
        """ selection_statement : IF LPAREN expression RPAREN statement ELSE statement """
        p[0] = c_ast.If(p[3], p[5], p[7], self._yacccoord(p,1))

    def p_selection_statement_3(self, p):
        """ selection_statement : SWITCH LPAREN expression RPAREN statement """
        p[0] = fix_switch_cases(
                c_ast.Switch(p[3], p[5], self._yacccoord(p,1)))

    def p_iteration_statement_1(self, p):
        """ iteration_statement : WHILE LPAREN expression RPAREN statement """
        p[0] = c_ast.While(p[3], p[5], self._yacccoord(p,1))

    def p_iteration_statement_2(self, p):
        """ iteration_statement : DO statement WHILE LPAREN expression RPAREN SEMI """
        p[0] = c_ast.DoWhile(p[5], p[2], self._yacccoord(p,1))

    def p_iteration_statement_3(self, p):
        """ iteration_statement : FOR LPAREN expression_opt SEMI expression_opt SEMI expression_opt RPAREN statement """
        p[0] = c_ast.For(p[3], p[5], p[7], p[9], self._yacccoord(p,1))

    def p_iteration_statement_4(self, p):
        """ iteration_statement : FOR LPAREN declaration expression_opt SEMI expression_opt RPAREN statement """
        p[0] = c_ast.For(c_ast.DeclList(p[3], self._yacccoord(p,1)),
                         p[4], p[6], p[8], self._yacccoord(p,1))

    def p_jump_statement_1(self, p):
        """ jump_statement  : GOTO ID SEMI """
        p[0] = c_ast.Goto(p[2], self._yacccoord(p,1))

    def p_jump_statement_2(self, p):
        """ jump_statement  : BREAK SEMI """
        p[0] = c_ast.Break(self._yacccoord(p,1))

    def p_jump_statement_3(self, p):
        """ jump_statement  : CONTINUE SEMI """
        p[0] = c_ast.Continue(self._yacccoord(p,1))

    def p_jump_statement_4(self, p):
        """ jump_statement  : RETURN expression SEMI
                            | RETURN SEMI
        """
        p[0] = c_ast.Return(p[2] if len(p) == 4 else None, self._yacccoord(p,1))

    def p_expression_statement(self, p):
        """ expression_statement : expression_opt SEMI """
        if p[1] is None:
            p[0] = c_ast.EmptyStatement(self._yacccoord(p,2))
        else:
            p[0] = p[1]
    ...
    def p_cast_expression_2(self, p):
        ...
        p[0] = c_ast.Cast(p[2], p[4], self._yacccoord(p,1))
    ...
    def p_postfix_expression_4(self, p):
        ...
        """
        field = c_ast.ID(p[3], self._yacccoord(p,3))
    ...
    def p_identifier(self, p):
        """ identifier  : ID """
        p[0] = c_ast.ID(p[1], self._yacccoord(p,1))
    ...
    def p_unified_string_literal(self, p):
        ...
        if len(p) == 2: # single literal
            p[0] = c_ast.Constant(
                'string', p[1], self._yacccoord(p,1))
        else:
